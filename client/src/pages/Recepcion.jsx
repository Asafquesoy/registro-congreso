  ...
  return (
    <div className={`persona ${persona.pagado ? 'persona--paid' : ''} reveal`}>
      ...
      <div className="persona-actions">
        {persona.pagado ? (
          <>
            <div className="status-pill status-paid">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              Pagado
            </div>
            <div className="status-pill status-attend">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              Asistencia
            </div>
            <button className="btn-undo" onClick={() => onPago(persona.id, false)}>
              Cancelar Pago
            </button>
          </>
        ) : (
          <>
            ...
            <button className="btn-pay" onClick={() => onPago(persona.id, true)}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              Registrar Pago y Asistencia
            </button>
          </>
        )}
      </div>
...
function RegistroSitio({ onRegistrado }) {
  ...
  const [form, setForm] = useState({ nombre: '', apellidos: '', iglesia: '', telefono: '', correo: '', taller: '' })
  const [error, setError] = useState('')
      ...
      {open && (
        <form className="sitio-form" onSubmit={handleSubmit}>
          ...
          <div className="sitio-grid">
            ...
            <input name="correo" placeholder="Correo (opcional)" value={form.correo} onChange={handleChange} />
            <select name="taller" value={form.taller} onChange={handleChange} required>
...
export default function Recepcion() {
  ...
  async function marcarPago(id, pagado) {
    const accion = pagado ? 'registrar el pago y asistencia' : 'cancelar el pago'
    if (!window.confirm(`¿Está seguro de ${accion} de esta persona?`)) return
      ...
      <main className="recep-main">
        ...
        <div className="results">
          ...
          {resultados && resultados.map(p => (
            <PersonaCard key={p.id} persona={p} onPago={marcarPago} />
          ))}
        </div>
      </main>
    </div>
